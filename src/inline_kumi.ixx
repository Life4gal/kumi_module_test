...
namespace kumi::detail
{
	...
	template<std::size_t I, typename T>
	constexpr T &get_leaf(leaf<I, T> &arg) noexcept
	{
		return arg.value;
	}
	template<std::size_t I, typename T>
...
export namespace kumi
{
	...
	template<std::size_t N>
	inline constexpr index_t<N> const index = {};
	namespace literals
	{
		template<char... c>
		constexpr auto b10()
	...
	[[nodiscard]] constexpr auto predicate() noexcept
	{
		return []<typename T>(T const &) constexpr
		{ return Pred<T>::value; };
	}
}// namespace kumi
...
export namespace kumi
{
	...
	template<typename T>
	struct size : std::tuple_size<T>
	{
	};
	template<typename T>
	struct size<T &> : size<T>
	{
	};
	template<typename T>
	struct size<T &&> : size<T>
	...
	template<std::size_t I, typename T>
	struct element : std::tuple_element<I, T>
	{
	};
	template<std::size_t I, typename T>
	struct element<I, T &> : element<I, T>
	{
	};
	template<std::size_t I, typename T>
	struct element<I, T &&> : element<I, T>
	...
	template<std::size_t I, typename T>
	struct element<I, T const &> : element<I, T>
	{
	};
	template<std::size_t I, typename T>
	struct element<I, T const &&> : element<I, T>
	...
	template<std::size_t I, typename T>
	using member_t = typename member<I, T>::type;
...
//	struct std::tuple_element<I, kumi::tuple<Ts...> const>
//	{
//		using type = typename tuple_element<I, kumi::tuple<Ts...>>::type const;
//	};
//	template<typename Head, typename... Tail>
//	struct std::tuple_element<0, kumi::tuple<Head, Tail...>>
//	{
//		using type = Head;
//	};
//	template<typename... Ts>
...
// Template specialization must open the namespace
// see: https://developercommunity.visualstudio.com/t/module-Template-specialization-must-op/10304585
export namespace std
{
	...
	template<typename... Ts>
	struct tuple_size<kumi::tuple<Ts...>> : integral_constant<size_t, sizeof...(Ts)>
	{
	};
}
...
namespace kumi::detail
{
	...
	struct is_piecewise_constructible;
	template<typename From, typename To>
	struct is_piecewise_convertible;
	...
	template<typename F, typename Indices, typename... Tuples>
...
export namespace kumi
{
	...
	template<typename T>
	concept std_tuple_compatible = empty_tuple<T> || non_empty_tuple<T>;
	template<typename T>
	...
	constexpr void for_each_index(Function f, Tuple &&t, Tuples &&...ts)
	{
		...
		else
		{
			[&]<std::size_t... I>(std::index_sequence<I...>)
			{
				...
				{
					f(idx, get<M::value>(KUMI_FWD(t)), get<M::value>(KUMI_FWD(ts))...);
				};
				(call(std::integral_constant<std::size_t, I>{}), ...);
			}(std::make_index_sequence<size<Tuple>::value>());
		}
...
export namespace kumi
{
	...
	struct tuple
	{
		...
		template<typename... Us>
			requires(detail::piecewise_convertible<tuple, tuple<Us...>> && (sizeof...(Us) == sizeof...(Ts)) && (!std::same_as<Ts, Us> && ...))
		[[nodiscard]] inline constexpr auto cast() const
		{
			return apply([](auto &&...elems)
						 { return tuple<Us...>{static_cast<Us>(elems)...}; },
						 *this);
		}
		template<typename... Us>
		...
		{
			[&]<std::size_t... I>(std::index_sequence<I...>)
			{ ((get<I>(*this) = get<I>(other)), ...); }(std::make_index_sequence<sizeof...(Ts)>());
			return *this;
		}
		template<typename... Us>
			requires(detail::piecewise_convertible<tuple, tuple<Us...>>)
		...
		constexpr decltype(auto) operator()(Function &&f) const &noexcept(noexcept(kumi::apply(KUMI_FWD(f), *this)))
		{
			return kumi::apply(KUMI_FWD(f), *this);
		}
#if !defined(KUMI_DOXYGEN_INVOKED)
		template<typename Function>
		constexpr decltype(auto) operator()(Function &&f) &noexcept(noexcept(kumi::apply(KUMI_FWD(f), *this)))
		{
			return kumi::apply(KUMI_FWD(f), *this);
		}
		template<typename Function>
...
export namespace kumi
{
	...
	[[nodiscard]] constexpr decltype(auto) front(Tuple &&t)
		requires(size_v<Tuple> != 0)
	{
		return get<0>(KUMI_FWD(t));
	}
	template<product_type Tuple>
	[[nodiscard]] constexpr decltype(auto) back(Tuple &&t)
		requires(size_v<Tuple> != 0)
	{
	...
	{
		template<product_type Tuple>
		struct front : member<0, Tuple>
		{
		};
		template<product_type Tuple>
		struct back : member<size_v<Tuple> - 1, Tuple>
		{
		};
		template<product_type Tuple>
...
export namespace kumi
{
	namespace detail
	{
		...
		constexpr auto digits(std::size_t v) noexcept
		{
			struct
			{
				std::size_t data[N];
...
#if !defined(KUMI_DOXYGEN_INVOKED)
	[[nodiscard]] constexpr auto cartesian_product()
	{
		return kumi::tuple<>{};
	}
#endif
...
export namespace kumi
{
	...
	[[nodiscard]] constexpr auto cat(Tuples &&...ts)
	{
		...
		else
		{
			...
			return [&]<std::size_t... N>(auto &&tuples, std::index_sequence<N...>)
			{
				...
				using type = kumi::tuple<std::tuple_element_t<pos.e[N], std::remove_cvref_t<std::tuple_element_t<pos.t[N], ts>>>...>;
...
export namespace kumi
{
	namespace detail
	{
		...
		struct as_tuple;
		template<product_type Tuple, std::size_t... I>
	...
	{
		return [&]<std::size_t... I>(std::index_sequence<I...>)
		{ return Type{get<I>(t)...}; }(std::make_index_sequence<sizeof...(Ts)>());
	}
	template<product_type Type>
	...
	struct as_tuple : detail::as_tuple<Tuple, std::make_index_sequence<kumi::size<Tuple>::value>, Meta>
	{
	};
...
export namespace kumi
{
	...
	[[nodiscard]] constexpr auto extract(Tuple const &t, [[maybe_unused]] index_t<I0> const &i0, [[maybe_unused]] index_t<I1> const &i1) noexcept
	{
		return [&]<std::size_t... N>(std::index_sequence<N...>)
		{
	...
	[[nodiscard]] constexpr auto extract(Tuple const &t, index_t<I0> const &i0) noexcept
	{
		return extract(t, i0, index<size_v<Tuple>>);
	}
	template<std::size_t I0, product_type Tuple>
		requires(I0 <= size_v<Tuple>)
	[[nodiscard]] constexpr auto split(Tuple const &t, [[maybe_unused]] index_t<I0> const &i0) noexcept
	{
...
{
	template<product_type Tuple>
	[[nodiscard]] constexpr auto flatten(Tuple const &ts)
	{
	...
	template<product_type Tuple>
	[[nodiscard]] constexpr auto flatten_all(Tuple &&ts)
	{
		if constexpr (sized_product_type<Tuple, 0>) return ts;
		else
		{
	...
	{
		return kumi::flatten_all(KUMI_FWD(ts), [](auto &m)
								 { return &m; });
	}
	namespace result
	{
...
export namespace kumi
{
	namespace detail
	{
		...
		constexpr auto const &eval(T const &v) noexcept
		{
			return v;
		}
	}// namespace detail
...
export namespace kumi
{
	...
	[[nodiscard]] constexpr auto inner_product(S1 const &s1, S2 const &s2, T init, Sum sum, Prod prod) noexcept
	{
		...
		else
		{
			return [&]<std::size_t... I>(std::index_sequence<I...>)
			{
				return (detail::foldable{sum, prod(get<I>(KUMI_FWD(s1)), get<I>(KUMI_FWD(s2)))} >> ... >> detail::foldable{sum, init}).value;
	...
	namespace result
	{
		...
		{
			using type = decltype(kumi::inner_product(std::declval<S1>(), std::declval<S2>(), std::declval<T>(), std::declval<Sum>(), std::declval<Prod>()));
		};
		template<product_type S1, sized_product_type<S1::size()> S2, typename T>
...
export namespace kumi
{
	...
	namespace result
	{
		...
		struct fold_right
		{
			using type = decltype(kumi::fold_right(std::declval<Function>(), std::declval<Tuple>(), std::declval<Value>()));
		};
		template<typename Function, product_type Tuple, typename Value>
		struct fold_left
		{
			using type = decltype(kumi::fold_left(std::declval<Function>(), std::declval<Tuple>(), std::declval<Value>()));
		};
		template<typename Function, product_type Tuple, typename Value>
...
export namespace kumi
{
	...
	{
		return count_if(ts, [](auto const &m)
	...
	[[nodiscard]] constexpr auto transpose(Tuple const &t)
	{
		...
		else
		{
			return [&]<std::size_t... I>(std::index_sequence<I...>)
			{
				...
				{
					return apply([](auto const &...m)
								 { return kumi::make_tuple(get<N::value>(m)...); },
								 u);
				};
